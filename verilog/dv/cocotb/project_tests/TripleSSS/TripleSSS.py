...
outputGPIO = 37 #Output signal to send to speaker (33+4)
...
async def send_pb(val):
    ...
    # negedge
    await cocotb.triggers.Timer(half_clk_period * 2 * 10, 'ns')
    caravelEnv.drive_gpio_in(val, 0x0)
    # negedge
    await cocotb.triggers.Timer(half_clk_period * 2 * 4, 'ns')
...
async def TripleSSS(dut):
    cocotb.log.info("[9] Start configuration")
    global caravelEnv
    caravelEnv = await test_configure(dut)
    cocotb.log.info("[9] Await mgmt_gpio")
    caravelEnv.drive_gpio_in((20,0), 0x0)
    await caravelEnv.release_csb()
    await caravelEnv.wait_mgmt_gpio(1)
    cocotb.log.info("[9] Finish configuration")
    global half_clk_period
    half_clk_period = caravelEnv.get_clock_period() / 2

    cocotb.log.info(f"[9] T1: Power-on-reset")
    # reset design
    caravelEnv.drive_gpio_in((20,0), 0x0)
    # wait 2 clk cycles
