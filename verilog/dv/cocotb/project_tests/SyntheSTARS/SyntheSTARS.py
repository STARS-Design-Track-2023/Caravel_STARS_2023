...
async def SyntheSTARS(dut):
    cocotb.log.info("[10] Start configuration")
    global caravelEnv
    caravelEnv = await test_configure(dut)
    cocotb.log.info("[10] Await mgmt_gpio")
    caravelEnv.drive_gpio_in((18,0), 0x0)
    await caravelEnv.release_csb()
    await caravelEnv.wait_mgmt_gpio(1)
    cocotb.log.info("[10] Finish configuration")
    global half_clk_period
    half_clk_period = caravelEnv.get_clock_period() / 2

    cocotb.log.info(f"[10] T1: Power-on-reset")
    # reset design
    caravelEnv.drive_gpio_in((18,0), 0x0)
    # wait 2 clk cycles
    ...
    cocotb.log.info(f"[10] Note: 6-2, wavelength[{int(await getNoteCycleCount())}/{int(27045)}], shape[{await getNoteShape()}/{int(1)}]")
